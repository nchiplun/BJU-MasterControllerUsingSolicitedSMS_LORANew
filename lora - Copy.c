...
void sendCmdToLora(unsigned char Action, unsigned char FieldNo) {
    ...
    switch (Action) {
    case 0x00:
        transmitStringToLora("#ON01TIMES");
        temporaryBytesArray[4] = (fieldValve[FieldNo].onPeriod%10) + 53;  // 5 count extra to lora auto timeout // fieldValve[FieldNo].onPeriod  Need to calculate on period for interrupted valve
        temporaryBytesArray[3] = (fieldValve[FieldNo].onPeriod/10);
        temporaryBytesArray[3] = (temporaryBytesArray[3]%10) + 48;
        temporaryBytesArray[2] = (fieldValve[FieldNo].onPeriod/100) + 48;
...
_Bool isLoraResponseAck(unsigned char Action, unsigned char FieldNo) {
    ...
    case 0x02:
        moistureLevel = CLEAR;
        /*
        for ( msgIndex = 1; msgIndex < 6 ; msgIndex++) {    // to fetch position of S in "#65535SLAVE01$" since freq. value can vary fro 1 to 5 digits
            //is number
