...
_Bool isFieldMoistureSensorWetLora(unsigned char FieldNo) {
    ...
    if (!LoraConnectionFailed && loraAttempt == 99 && !moistureSensorFailed) {  // Successful Sensor reading
        /*
        for ( msgIndex = 1; msgIndex < 6 ; msgIndex++) {
            ...
            if (isNumber(decodedString[msgIndex])) {
                if (decodedString[msgIndex + 1] == 'S') {
                    ...
                    digit = digit + decodedString[msgIndex];
                    msgIndex = 99; // break
                    sendSms("break/r/n", userMobileNo, noInfo); // Acknowledge user about successful action
                } 
                else {
                    decodedString[msgIndex] = decodedString[msgIndex] - 48;
                    decodedString[msgIndex] = decodedString[msgIndex] * 10;
        ...
        */
        //moistureLevel = digit;
        sendSms("moisture/r/n", userMobileNo, noInfo); // Acknowledge user about successful action
    }
    else {
        moistureLevel = CLEAR;
    ...
    if (moistureLevel >= fieldValve[FieldNo].wetValue) { //Field is full wet, no need to switch ON valve and motor, estimate new due dates
        ...
        //********Debug log#end**************//
        #endif
        sendSms("wet/r/n", userMobileNo, noInfo); // Acknowledge user about successful action
        return true;
    } 
    else {
        #ifdef DEBUG_MODE_ON_H
...
void actionsOnSystemReset(void) {
    ...
    for (iterator = 0; iterator < 2; iterator++) {
        fieldValve[iterator].onPeriod = 2;
        fieldValve[iterator].offPeriod = 1;
        ...
        fieldValve[iterator].dryValue = 100;
        fieldValve[iterator].wetValue = 3000;
        fieldValve[iterator].priority = iterator+1;
        fieldValve[iterator].status = OFF;
