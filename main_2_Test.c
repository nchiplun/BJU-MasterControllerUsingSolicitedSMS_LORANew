...
void __interrupt(low_priority) timerInterrupt_handler(void) {
    ...
    if (PIR0bits.TMR0IF) {
        ...
        */
        ...
        if (!fieldValve[9].isConfigured && !fieldValve[10].isConfigured && !fieldValve[11].isConfigured && !fieldValve[12].isConfigured) { // 8-12 used as injector
            //To follow fertigation cycle sequence
            if (fertigationValveControl == ON) {
        ...
        else {                                  // Filtration is Enabled
            if (filtrationCycleSequence == 1 && Timer0Overflow == filtrationDelay1 ) { // Filtration1 Start Delay
                Timer0Overflow = 0;
