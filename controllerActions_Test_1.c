...
_Bool isFieldMoistureSensorWet(unsigned char FieldNo) {
    ...
    for (itr = 1; itr <= avg && !moistureSensorFailed; itr++) {
        ...
        case 0:
            ...
            T5CONbits.TMR5ON = ON; // Start Timer after rising edge detected
        ...
        T5CONbits.TMR5ON = OFF; // Stop timer after falling edge detected
...
_Bool isRTCBatteryDrained(void) {
    ...
    unsigned int batteryVoltageCutoff = 555;   //555 ---- ~2.7 v
    #ifdef DEBUG_MODE_ON_H
    //********Debug log#start************//
...
void configureController(void) {
    ...
    //-----------Timer1_Config (15 sec) used to control fertigation injector sequence-----------//
    //-----------Timer will not halt in sleep mode------------------------------------------------------//

    T1CON = 0b00110110; // 16 bit Timer with asynchronous mode with 1:8 pre scale
    TMR1CLK = 0b00000100; // Clock source as LFINTOSC
    TMR1H = 0x1C; // Load Timer3 Register Higher Byte 
    TMR1L = 0xF2; // Load Timer3 Register lower Byte 
    PIR5bits.TMR1IF = CLEAR; // Clear Timer3 Overflow Interrupt at start
    PIE5bits.TMR1IE = ENABLED; // Enables the Timer3 Overflow Interrupt
    IPR5bits.TMR1IP = LOW; // Low Timer3 Overflow Interrupt Priority
    ...
    //-----------Timer5_Config used for calculation pulse width of moisture sensor output-----------//
    //-----------Timer will halt in sleep mode------------------------------------------------------//

    T5CON = 0b00000010; // 16 bit Timer with Synchronous mode
    TMR5CLK = 0b00000001; //  Clock source as FOSC/4
    TMR5H = CLEAR; // Clear Timer5 Register Higher Byte
    TMR5L = CLEAR; // Clear Timer5 Register Lower Byte
    PIR5bits.TMR5IF = CLEAR; // Clear Timer5 Overflow Interrupt at start
    PIE5bits.TMR5IE = ENABLED; // Enables the Timer5 Overflow Interrupt
    IPR5bits.TMR5IP = LOW; // Low Timer5 Overflow Interrupt Priority
    ...
    OSCENbits.SOSCEN = ENABLED; //Secondary Oscillator is enabled
...
void actionsOnSystemReset(void) {
    ...
    for (iterator = 0; iterator < 2; iterator++) {
        fieldValve[iterator].onPeriod = 994;
        ...
        fieldValve[iterator].fertigationDelay = 2;
        ...
        fieldValve[iterator].isFertigationEnabled = false; // true;
