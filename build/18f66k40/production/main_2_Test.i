    ...
    if (PIR4bits.RC3IF) {
        ...
        if (inSleepMode) {
            ...
            if (rxCharacter == '+') {
                ...
                cmtiCmd= 0;
                temporaryBytesArray[0] = rxCharacter;
                msgIndex = 0;
                newSMSRcvd = 1;
        ...
        if ((IOCEF5 == 1 || IOCEF6 == 1 || IOCEF7 == 1)) {
            myMsDelay(5000);
            if (PORTEbits.RE5 == 0 && PORTEbits.RE6 == 0 && PORTEbits.RE7 == 0) {
...
void __attribute__((picinterrupt(("low_priority")))) timerInterrupt_handler(void) {
    ...
    if (PIR0bits.TMR0IF) {
        ...
        if (sleepCount > 0 && PORTFbits.RF7 == 1) {
            ...
            if (loraAliveCountCheck <= loraAliveCount+1) {





                sendSms("Lora is alive", userMobileNo, 0);
            }
            else {
                ...
                sendSms("Lora is not alive", userMobileNo, 0);
            }
        }
# 213 "main_2_Test.c"
        if (!fieldValve[9].isConfigured && !fieldValve[10].isConfigured && !fieldValve[11].isConfigured && !fieldValve[12].isConfigured) {
...
nxtVlv: if (!valveDue && !phaseFailureDetected && !lowPhaseCurrentDetected) {
        ...
        if (valveDue && dueValveChecked) {
            ...
            dueValveChecked = 0;
            sendSms("due valve", userMobileNo, 0);
